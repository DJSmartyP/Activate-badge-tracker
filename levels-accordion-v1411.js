(() => {
  ...
  function appendGameOptions(group, names) {
    names.forEach(name => {
  ...
  function groupGameDropdown() {
    ...
    names.forEach(name => {
      ...
      // If a title exists in both modes, list it once under CO-OP. Selecting
      // it still filters by the shared game name, so no functionality is lost.
    ...
    if (cooperative.length) {
      ...
      group.label = 'CO-OP';
      appendGameOptions(group, cooperative);
      fragment.appendChild(group);
    }

    if (competitive.length) {
      const group = document.createElement('optgroup');
      group.label = 'COMPETITIVE';
      appendGameOptions(group, competitive);
      fragment.appendChild(group);
    }
