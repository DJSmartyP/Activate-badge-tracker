(() => {
  ...
  let enhancing = false;
  let controls = null;
  const gameModesByRoom = new Map();

  function text(el) {
    return (el?.textContent || '').trim();
  }

  function rememberGameMode(room, game, mode) {
    if (!room || !game) return;
    if (!gameModesByRoom.has(room)) gameModesByRoom.set(room, new Map());
    const roomMap = gameModesByRoom.get(room);
    const current = roomMap.get(game) || { cooperative: false, competitive: false };
    if (mode === 'cooperative') current.cooperative = true;
    if (mode === 'competitive') current.competitive = true;
    roomMap.set(game, current);
  }

  function cacheGameModesFromProgress() {
    const progressEntriesFn = globalThis.progressEntries;
    if (typeof progressEntriesFn !== 'function') return;
    try {
      const entries = progressEntriesFn();
      if (!Array.isArray(entries)) return;
      entries.forEach(entry => rememberGameMode(entry?.room, entry?.game, entry?.mode));
    } catch (_) {
      // DOM inspection below remains as a safe fallback.
    }
  }

  function cacheGameModesFromDom(list) {
    [...list.querySelectorAll(':scope > .room-section, :scope > details.levels-room-accordion')].forEach(section => {
      const room = section.dataset.room ||
        text(section.querySelector(':scope > .room-title-row h3')) ||
        text(section.querySelector(':scope > summary .levels-room-name'));
      if (!room) return;

      section.querySelectorAll('.room-game-card, details.levels-game-accordion').forEach(card => {
        const game = card.dataset.game || text(card.querySelector(':scope > .room-game-head strong')) ||
          text(card.querySelector(':scope > summary .room-game-head strong'));
        if (!game) return;
        if (card.classList.contains('coop-game') || card.classList.contains('levels-game-accordion')) {
          rememberGameMode(room, game, 'cooperative');
        }
        if (card.classList.contains('comp-game')) {
          rememberGameMode(room, game, 'competitive');
        }
      });
    });
  }

  function appendGameOptions(fragment, names) {
    names.forEach(name => {
  ...
  function appendSeparator(fragment, label) {
  ...
  function groupGameDropdown() {
    const roomSel = document.getElementById('levelsRoom');
    const gameSel = document.getElementById('levelsGame');
    const room = roomSel?.value || '';
    if (!gameSel || !room || gameSel.disabled) return;

    const currentGame = gameSel.value || '';
    const names = [...new Set([...gameSel.querySelectorAll('option')]
      .map(option => option.value)
      .filter(Boolean))];
    if (!names.length) return;

    const roomModes = gameModesByRoom.get(room) || new Map();
    const cooperative = [];
    const competitive = [];
    const other = [];

    names.forEach(name => {
      const mode = roomModes.get(name);
      // If a title exists in both modes, list it once under CO-OP.
      if (mode?.cooperative) cooperative.push(name);
      else if (mode?.competitive) competitive.push(name);
      else other.push(name);
    });

    cooperative.sort((a,b) => a.localeCompare(b));
    competitive.sort((a,b) => a.localeCompare(b));
    ...
    if (cooperative.length) {
      appendSeparator(fragment, 'CO-OP');
      appendGameOptions(fragment, cooperative);
    }

    if (competitive.length) {
      appendSeparator(fragment, 'COMPETITIVE');
      appendGameOptions(fragment, competitive);
    }

    // Safety fallback for any game whose mode cannot be identified.
    // Keep it selectable rather than losing it from the filter.
    if (other.length) {
      appendSeparator(fragment, 'OTHER');
      appendGameOptions(fragment, other);
    }

    gameSel.replaceChildren(fragment);
    if (currentGame && names.includes(currentGame)) gameSel.value = currentGame;
    else gameSel.value = '';
    gameSel.dataset.groupedRoom = room;
  }
  ...
  function renderFlatHighScores(list) {
    ...
    sections.forEach(section => {
      ...
      section.querySelectorAll(':scope > .room-games > .room-game-card.coop-game').forEach(card => {
        ...
        const scores = [...card.querySelectorAll('.level-chip.high-score-level')].map(chip => ({
          level: text(chip.querySelector('.level-chip-title-row strong')) || 'Level',
          score: highScoreValue(chip)
        }));
        if (scores.length) games.push({ game, scores });
      });

      if (games.length) roomGroups.push({ room, games });
    });

    if (!roomGroups.length) return false;

    list.innerHTML = `<div class="levels-highscore-flat" aria-label="Venue high scores">${roomGroups.map(roomGroup => `
      <section class="levels-highscore-room-group">
        <header class="levels-highscore-room-heading">
          <span>Room</span>
          <strong>${escapeHtml(roomGroup.room)}</strong>
        </header>
        <div class="levels-highscore-room-games">${roomGroup.games.map(gameGroup => `
          <section class="levels-highscore-game-group">
            <header class="levels-highscore-game-heading">
              <span>Game</span>
              <strong>${escapeHtml(gameGroup.game)}</strong>
            </header>
            <div class="levels-highscore-game-rows">${gameGroup.scores.map(row => `
              <article class="levels-highscore-row level-chip high-score-level">
                <div class="levels-highscore-copy">
                  <strong class="levels-highscore-level">${escapeHtml(row.level)}</strong>
                </div>
  ...
  function enhanceLevels() {
    ...
    cacheGameModesFromProgress();
    cacheGameModesFromDom(list);
    groupGameDropdown();
  ...
  function start() {
    ...
    ['levelsRoom', 'levelsGame', 'levelsView', 'gamesView'].forEach(id => {
      document.getElementById(id)?.addEventListener('change', () => queueMicrotask(enhanceLevels));
    });
    document.getElementById('levelsModeTabs')?.addEventListener('click', () => queueMicrotask(enhanceLevels));
